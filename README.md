# estudos-python

Repositório dedicado à minha jornada de aprendizado em Python — do zero ao domínio da linguagem.

---

## Sobre mim

Sou Lukas, desenvolvedor em formação. Este repositório é o meu diário de estudos público: aqui documento cada etapa do meu aprendizado em Python, desde os fundamentos até projetos mais complexos.

O objetivo é evoluir de forma consistente e transparente — mostrando não só o código final, mas toda a progressão até chegar lá.

---

## Objetivos de Aprendizado

Acompanhe minha trilha de estudos. Os tópicos marcados já foram concluídos:

### Fundamentos
- [x] Variáveis, tipos de dados e operadores
- [x] Estruturas condicionais e de repetição
- [x] Funções e escopo
- [x] Listas, tuplas, dicionários e conjuntos

### Tratamento de Erros
- [x] `try`, `except`, `finally`
- [x] Tipos de exceções e hierarquia de erros
- [x] Criação de exceções customizadas

### Manipulação de Arquivos
- [x] Leitura e escrita de arquivos `.txt`
- [ ] Formato JSON e biblioteca `json`
- [ ] Manipulação de arquivos `.json`

### Orientação a Objetos (POO)
- [ ] Classes e objetos
- [ ] Herança e polimorfismo
- [ ] Encapsulamento e abstração
- [ ] Métodos especiais (dunder methods)

### Boas Práticas
- [ ] PEP 8 — guia de estilo Python
- [ ] Docstrings e documentação de código
- [ ] Organização de projetos e nomenclatura

---

## Mini Projetos & Exercícios

Em breve — os projetos serão adicionados aqui conforme forem sendo desenvolvidos.

| Projeto | Descrição | Tópicos abordados | Status |
|---------|-----------|-------------------|--------|
| [Agenda de Contatos](exercicios-de-manipulacao-de-arquivos/ex001.py) | Agenda simples via terminal com leitura, escrita e remoção de contatos em arquivo `.txt` | Manipulação de arquivos, funções, loops, tratamento de erros | Concluído |

---

## Tecnologias Utilizadas

- Python 3
- Git e GitHub
- VS Code

---

Feito com dedicação por Lukas.
